...
> *Are the actions suggested by an AI model to prevent employee attrition equally achievable for all demographic groups (e.g., gender, age), and does this fairness depend on the type of ML model used?*
...
```
IBM HR Dataset → Preprocessing → ML Models (≥2) → DiCE Counterfactuals → FACTS Fairness Analysis → Results & Report
```
